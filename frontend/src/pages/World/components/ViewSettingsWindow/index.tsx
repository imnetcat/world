...
export interface ViewSettings {
    tileSize: number;
    mode: 'terrain' | 'height' | 'moisture' | 'temperature';
    offsetX: number;
    offsetY: number;
    zoom: number;
}
...
export const ViewSettingsWindow = ({ worldHeight, worldWidth, loadTime, renderTime, viewSettings, setViewSettings }: ViewSettingsWindowProps) => {
            ...
            >
                <Flex.Col fullHeight fullWidth wrap gap={16}>
                    <Flex.Row flex={0} fullWidth gap={16}>
                        <Flex.Col>
                            <Flex.Row gap={4}>
                                <Text size={2} type='secondary'>Last Load time:</Text>
                                <Text size={2}>{!loadTime ? '-' : `${loadTime / 1000} sec`}</Text>
                            </Flex.Row>
                        </Flex.Col>
                        <Flex.Col>
                            <Flex.Row gap={4}>
                                <Text size={2} type='secondary'>Last Render time:</Text>
